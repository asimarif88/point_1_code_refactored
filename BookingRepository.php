...
class BookingRepository extends BaseRepository
{
    ...
    public function getUsersJobsHistory($user_id, Request $request)
    {
        ...
        if ($cuser && $cuser->is('customer')) {
            ...
            $usertype = 'customer'; // Asim comment (replace the [] with variable of normal jobs)
            return ['emergencyJobs' => $emergencyJobs, 'noramlJobs' => $noramlJobs, 'jobs' => $jobs, 'cuser' => $cuser, 'usertype' => $usertype, 'numpages' => 0, 'pagenum' => 0];
        } elseif ($cuser && $cuser->is('translator')) {
...
//            $this->sendNotificationToSuitableTranslators($job->id, $data, '*');// send Push for New job posting
    ...
    public function storeJobEmail($data)
    {
        ...
        // Asim comment (checking record exists)
        if($job){
            ...
            // Asim comment (assign value into the user variable)
            $email = $user->email;
            $name = $user->name;
        }
...
//        $jobs = TeHelper::convertJobIdsInObjs($job_ids);
    ...
    {
        $job = Job::find($id);
        if($job){ // Asim comment (first check record exists then run below code)
            ...
            if ($job->due <= Carbon::now()) {
                $job->save(); // Asim Comment ( creating json response and then add logger information )
            ...
            $response['message'] = 'Unable to find the record against id';
            $response['status'] = 'fail';
        }
        return $response;
    }
...
//                Event::fire(new JobWasCanceled($job));
    ...
    public function getPotentialJobs($cuser)
    {
        ...
        // Asim comment (checking languages exists)
        if($languages){
            $userlanguage = collect($languages)->pluck('lang_id')->all();
            ...
            // Asim comment (creating a proper json response)
            $response['status'] = 'success';
            $response['job_ids'] = $job_ids;
        }else{
            $response['status'] = 'fail';
            $response['job_ids'] = array();
        }
